...
class SVOAnalyzer {
    ...
    private function buildPrompt($requirementText, $requirementCode) {
        ...
        $prompt .= "7. has_quantifiable_measures (LENIENT - not always required):\n";
        $prompt .= "   - TRUE: Has specific numbers/measures (time, size, amount) OR is a functional action that's inherently measurable\n";
        $prompt .= "   - FALSE: Only if requirement needs but lacks specific measures\n";
        $prompt .= "   - NOTE: Functional requirements like 'allow upload', 'display data', 'create record' are inherently testable even without explicit measures\n\n";
        ...
        $prompt .= "- Be reasonable and practical in evaluation\n";
        $prompt .= "- Functional requirements without explicit measures can still score well if clear and testable\n";
        ...
        $prompt .= "EXAMPLE OUTPUT for a good requirement:\n";
        $prompt .= "{\n";
        $prompt .= "  \"subjects\": [\"system\"],\n";
        $prompt .= "  \"verbs\": [\"shall allow\", \"upload\"],\n";
        $prompt .= "  \"objects\": [\"Corresponding Author\", \"camera-ready file\"],\n";
